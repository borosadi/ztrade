...
@click.option('--algorithmic', is_flag=True, help='Algorithmic mode: use rules-based decisions (sentiment + technical only, no AI)')
def run(agent_id, dry_run, manual, subagent, automated, algorithmic):
    ...
    if algorithmic:
        click.echo(f"\n📊 ALGORITHMIC MODE - Using rules-based decisions (sentiment + technical analysis)\n")

    click.echo(f"Running trading cycle for {agent_id} ({asset})...")

    # Check if agent is active
    if status != 'active' and not dry_run and not manual and not subagent and not automated and not algorithmic:
...
Respond with ONLY the JSON object, nothing else."""
            ...
            click.echo("4. Making algorithmic decision (sentiment + technical)...")
            ...
            try:
                ...
                decision_maker = get_algorithmic_decision_maker(
                    sentiment_weight=0.6,  # 60% weight on sentiment
                    technical_weight=0.4   # 40% weight on technical
                )

                # Extract the data we need for algorithmic decision
                ...
                performance_metrics['decision_ms'] = (time.time() - start_time) * 1000

                click.echo(f"   Decision: {decision.get('decision', 'unknown').upper()}")
                click.echo(f"   Confidence: {decision.get('confidence', 0)*100:.0f}%")
                click.echo(f"   Rationale: {decision.get('rationale', 'N/A')}")
                click.echo(f"   Decision time: {performance_metrics['decision_ms']:.1f}ms")

            except Exception as e:
                click.echo(f"\n✗ Algorithmic decision failed: {e}", err=True)
                logger.error(f"Algorithmic decision error: {e}", exc_info=True)
                return
            ...
            click.echo("1. Algorithmic mode: ztrade agent run {} --algorithmic (RECOMMENDED - no AI, fast, deterministic)".format(agent_id))
            click.echo("2. Automated mode: ztrade agent run {} --automated (requires ANTHROPIC_API_KEY)".format(agent_id))
            click.echo("3. Subagent mode: ztrade agent run {} --subagent (within Claude Code terminal)".format(agent_id))
            click.echo("4. Manual mode: ztrade agent run {} --manual (interactive)".format(agent_id))
            click.echo("\nAlgorithmic mode is RECOMMENDED for production trading (fast, cheap, predictable).")
            click.echo("Automated mode is best for AI-driven adaptive strategies.\n")
            return
