...
@click.option('--algorithmic', is_flag=True, help='Use algorithmic mode (rules-based, RECOMMENDED for production)')
@click.option('--market-hours/--no-market-hours', default=True, help='Only trade during market hours (default: on)')
@click.option('--detach', is_flag=True, help='Run in detached mode (returns immediately, no monitoring)')
def start(agent_id, interval, max_cycles, dry_run, manual, subagent, automated, algorithmic, market_hours, detach):
    ...
    if success:
        ...
        if detach:
            ...
            click.echo(f"   Mode: detached (running in background)")
            click.echo(f"\n   Use 'ztrade loop status {agent_id}' to check status")
            click.echo(f"   Use 'ztrade loop stop {agent_id}' to stop the loop\n")
        else:
            # Attached mode: monitor the loop
            click.echo(f"   Mode: attached (monitoring loop)")
            ...
            try:
                while True:
                    time.sleep(30)  # Check every 30 seconds (reduced from 0.5s to avoid CPU spin)
