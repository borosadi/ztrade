...
2. Analyze sentiment (news, reddit, Elon Musk tweets, meme trends)
...
def is_market_hours(**context):
    """DOGE trades 24/7 - market is always open for crypto."""
    ...
    logger.info(f"[{AGENT_ID}] Market hours check: {now.strftime('%Y-%m-%d %H:%M:%S')} EST - DOGE trades 24/7")

    # Crypto markets never close
    return True

def fetch_market_data(**context):
    """Task 1: Fetch current market data for DOGE/USD and save to database."""
...
# Define the DAG
dag = DAG(
    dag_id='agent_doge_trading',
    default_args=default_args,
    description='DOGE/USD sentiment_momentum trading agent - 1-hour cycles',
