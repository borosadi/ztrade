...
def is_market_hours(**context):
    """BTC trades 24/7 - market is always open for crypto."""
    ...
    logger.info(f"[{AGENT_ID}] Market hours check: {now.strftime('%Y-%m-%d %H:%M:%S')} EST - BTC trades 24/7")

    # Crypto markets never close
    return True

def fetch_market_data(**context):
    """Task 1: Fetch current market data for BTC/USD and save to database."""
...
# Define the DAG
dag = DAG(
    dag_id='agent_btc_trading',
    default_args=default_args,
    description='BTC/USD sentiment_momentum trading agent - 1-hour cycles',
