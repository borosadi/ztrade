...
2. Analyze sentiment (news, reddit, SEC lawsuit/regulatory narratives)
...
def is_market_hours(**context):
    """XRP trades 24/7 - market is always open for crypto."""
    ...
    logger.info(f"[{AGENT_ID}] Market hours check: {now.strftime('%Y-%m-%d %H:%M:%S')} EST - XRP trades 24/7")

    # Crypto markets never close
    return True

def fetch_market_data(**context):
    """Task 1: Fetch current market data for XRP/USD and save to database."""
...
# Define the DAG
dag = DAG(
    dag_id='agent_xrp_trading',
    default_args=default_args,
    description='XRP/USD sentiment_momentum trading agent - 1-hour cycles',
    schedule_interval=f'0 */{INTERVAL_MINUTES} * * *',  # Every 60 min (at minute 0), 24/7
    start_date=days_ago(1),
    catchup=False,
    max_active_runs=1,
    tags=['trading', 'xrp', 'crypto', 'regulatory', 'sentiment_momentum', 'algorithmic'],
)
